...
cd "{run_folder_td}"
srun --exclusive --ntasks=1 --cpus-per-task=64 \
    ./dmrg -f "{input_path.name}" -p "{DMRG_PRECISION}"

for step_folder in "{run_folder_twotime}"/step_*; do
    (
        cd "$step_folder"
        srun --exclusive --ntasks=1 --cpus-per-task={args.cpus_per_task} \
            ./dmrg -f "input_{run_name}_apply.ain" -p "{DMRG_PRECISION}"
        srun --exclusive --ntasks=1 --cpus-per-task={args.cpus_per_task} \
            ./dmrg -f "input_{run_name}_evolve.ain" -p "{DMRG_PRECISION}" \
            "{OPERATOR_LABEL}"
    ) &
done
