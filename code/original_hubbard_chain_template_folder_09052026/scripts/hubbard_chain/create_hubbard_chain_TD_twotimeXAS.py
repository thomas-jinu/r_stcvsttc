...
def main():
    ...
    Pump_numberoftimesteps = conda_dmrg.ask_int(
        "Number of physical time steps, including t=0 (Pump_numberoftimesteps)", 101
    )
    ...
    """
    Here you now create the folder structure
    """
    ...
    """
    Create the two-time grid for XAS calculations
    """
    Greens_function = conda_dmrg.ask_text(
        "Greens function type (Greens_function)", "XAS"
    )

    if Greens_function == "XAS":
        print("\n=== Two-time XAS calculation ===")

        for t_prime in range(Pump_numberoftimesteps - 1):
            dir_name_tp = f"tp{t_prime:04d}"
            run_folder_tp, executable_path = conda_dmrg.prepare_run_folder(
                run_folder / dir_name_tp,
                executable_source=DMRG_EXECUTABLE_SOURCE,
                root=inherited_input_path.parent,
                executable_name="dmrg",
            )
            ...
            """
            Create the reference ainur input;
            """
            if t_prime > 0:
                """
                Time evolve the system to the next time step and use that as the reference for the next time step.

                Should create a reference folder inside the current folder, and copy the reference input file from the previous time step into it.
                """
                dir_name_tp_reference = "reference_" + dir_name_tp

                run_folder_tp_reference, executable_path = (
                    conda_dmrg.prepare_run_folder(
                        run_folder_tp / dir_name_tp_reference,
                        executable_source=DMRG_EXECUTABLE_SOURCE,
                        root=inherited_input_path.parent,
                        executable_name="dmrg",
                    )
            ...
            """
            Create the insert ainur input;

            Things in the input script that has to change;

            potentialV;
            center_site;
            SolverOptions; Targeting expression;
            OuptutFile; 
            FiniteLoops;
            RepeatFiniteLoopsTimes;
            TSPAdvanceEach;
            RestartMapStages=0;
            string P0="c'[0]*|gs>";
            GsWeight=0.1;
            ...
            """
            Create the evolve ainur input
            """

        output_path = (
            run_folder_tp / f"input_{run_name}.ain"
            if run_folder_tp is not None
        ...
        if run_folder_tp is not None:
            pump_output_path = run_folder_tp / PUMP_OUTPUT_FILENAME
            conda_dmrg.write_pump_table(Pump_table, pump_output_path)
            print(f"\nCreated TD run folder: {run_folder_tp}")
    ...
    If it is run now then a run_all.sh should queue, otherwise a batch script should be created for the user to run later.
    """
...
./dmrg -f  input_{run_name}.ain
            ...
            slurm_script_path = run_folder_tp / f"batch_{run_name}.slurm"
